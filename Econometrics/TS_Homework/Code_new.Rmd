...
GDP <- import('/Users/maxim_anisimov/Desktop/TS_data/homework/dirty_data/Real_GDP-2.csv')
CPI <- import('//Users/maxim_anisimov/Desktop/TS_data/homework/dirty_data/CPI-2.csv')
BondYield10Y <- import('/Users/maxim_anisimov/Desktop/TS_data/homework/dirty_data/Bond_Yield_10Y-2.csv')
BondYield3M <- import('/Users/maxim_anisimov/Desktop/TS_data/homework/dirty_data/yield3_2.csv')
...
adf.test(log(ts_train$GDP)) # not stationary
adf.test(na.omit(diff(ts_train$GDP)))
adf.test(na.omit(diff(log(ts_train$GDP))))
...
adf.test(log(ts_train$CPI)) # stationary
adf.test(na.omit(diff(log(ts_train$CPI)))) # stationary

ts_stationary_data <- cbind(na.omit(diff(log(ts_data$GDP))),
                            na.omit(diff(ts_data$BondYieldSpread)),
                           log(ts_data$CPI[2:nrow(ts_data)]))
colnames(ts_stationary_data) <- c('log_GDP_diff', 'BondYieldSpread_diff', 'log_CPI')
...
var_model1 <- VAR(as.ts(ts_stationary_train), p = 1, type = "const")
serial.test(var_model1, lags.pt = 12, type = "PT.asymptotic") # no corr  
serial.test(var_model1, lags.pt = 12, type = "BG") # no corr

var_model2 <- VAR(as.ts(ts_stationary_train), p = 2, type = "const")
serial.test(var_model2, lags.pt = 12, type = "PT.asymptotic") # no corr
serial.test(var_model2, lags.pt = 12, type = "BG") # no corr
...
normality.test(var_model1, multivariate.only = FALSE)
normality.test(var_model2, multivariate.only = FALSE)
normality.test(var_model3, multivariate.only = FALSE)
# errors are not normal in both models
```

```{r Forecasting, echo=FALSE}
var_model1_fcst <- forecast(var_model1, h = test_size)
var_model2_fcst <- forecast(var_model2, h = test_size)
var_model3_fcst <- forecast(var_model3, h = test_size)

# only MASE
accuracy(snaive(ts_stationary_train$log_GDP_diff, h=test_size), ts_stationary_test$log_GDP_diff)[,6]
accuracy(var_model1_fcst$forecast$log_GDP_diff, ts_stationary_test$log_GDP_diff)[,6]
accuracy(var_model2_fcst$forecast$log_GDP_diff, ts_stationary_test$log_GDP_diff)[,6]
accuracy(var_model3_fcst$forecast$log_GDP_diff, ts_stationary_test$log_GDP_diff)[,6]

accuracy(snaive(ts_stationary_train$log_CPI, h=test_size), ts_stationary_test$log_CPI)[,6]
accuracy(var_model1_fcst$forecast$log_CPI, ts_stationary_test$log_CPI)[,6]
accuracy(var_model2_fcst$forecast$log_CPI, ts_stationary_test$log_CPI)[,6]
accuracy(var_model3_fcst$forecast$log_CPI, ts_stationary_test$log_CPI)[,6]

accuracy(naive(ts_stationary_train$BondYieldSpread_diff, h=test_size), ts_stationary_test$BondYieldSpread_diff)[,6]
accuracy(var_model1_fcst$forecast$BondYieldSpread_diff, ts_stationary_test$BondYieldSpread_diff)[,6]
accuracy(var_model2_fcst$forecast$BondYieldSpread_diff, ts_stationary_test$BondYieldSpread_diff)[,6]
accuracy(var_model3_fcst$forecast$BondYieldSpread_diff, ts_stationary_test$BondYieldSpread_diff)[,6]

VAR_plot_fsct <- autoplot(var_model2_fcst)
VAR_plot_actual <- autoplot(ts_stationary_data)
gdp_plot_fcst <- autoplot(var_model2_fcst$forecast$log_GDP_diff)
cpi_plot_fcst <- autoplot(var_model2_fcst$forecast$log_CPI)
bondyieldspread_plot_fcst <- autoplot(var_model2_fcst$forecast$BondYieldSpread_diff)
...
# changing order
ts_var_2_train <- cbind(ts_stationary_train$log_GDP_diff, ts_stationary_train$log_CPI, ts_stationary_train$BondYieldSpread_diff)

var_2 <- VAR(ts_var_2_train, p = 2, type = "const")
summary(var_2)$covres
summary(var_2)$corres
# Here we can just show Cholesky decomposed matrix B^{-1}
Omega_hat <- summary(var_2)$covres
# these two matrices must be indentical
chol(Omega_hat)%>%t()
Psi(var_2)[,,1]

# ortho = TRUE - usage of Choleski decomposition (default)
plot(irf(var_2, impulse = "log_GDP_diff", response = c("log_CPI"), n.ahead = test_size, cumulative=F))
plot(irf(var_2, impulse = "log_GDP_diff", response = c("BondYieldSpread_diff"), n.ahead = test_size, cumulative=T))
plot(irf(var_2, impulse = "log_GDP_diff", response = c("log_GDP_diff"), n.ahead = test_size, cumulative = TRUE))

plot(irf(var_2, impulse = "log_CPI", response = c("log_CPI"), n.ahead = test_size+8, cumulative = F))
plot(irf(var_2, impulse = "log_CPI", response = c("BondYieldSpread_diff"), n.ahead = test_size, cumulative = TRUE))
plot(irf(var_2, impulse = "log_CPI", response = c("log_GDP_diff"), n.ahead = test_size, cumulative = TRUE))

plot(irf(var_2, impulse = "BondYieldSpread_diff", response = c("BondYieldSpread_diff"), n.ahead = test_size, cumulative = TRUE))
plot(irf(var_2, impulse = "BondYieldSpread_diff", response = c("log_CPI"), n.ahead = test_size+8, cumulative = F))
plot(irf(var_2, impulse = "BondYieldSpread_diff", response = c("log_GDP_diff"), n.ahead = test_size, cumulative = TRUE))
...
svarfevd <- fevd(var_2, n.ahead = test_size)
svarfevd
plot.varfevd(svarfevd, plot.type = "single", col=1:3, legend=F)
```